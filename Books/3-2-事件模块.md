...
bool GameEvent.HasEventListener<T1>(int eventType, Action<T1> handler);
// ... 最多 HasEventListener<T1,T2,T3,T4,T5,T6>
bool GameEvent.HasEventListener(int eventType, Delegate handler);  // Delegate 重载
...
#### RemoveAllListeners（按事件 ID 清空所有监听）

无需持有注册时的委托，凭事件 ID 清空该事件下的全部监听，不影响其他事件。回调遍历过程中调用也安全（走延迟删除）。

```csharp
GameEvent.RemoveAllListeners(int eventType);
GameEvent.RemoveAllListeners(string eventType);
GameEvent.RemoveAllListeners(IGameSceneEvent.OnSceneLoadOver);  // 接口事件生成 ID
```

> **注意**：这是按事件 ID 清空**全部**监听，不是按对象定向解绑。组件自身能持有委托时仍优先用 `RemoveEventListener` 精确移除；对公共事件使用前要确认不会误删其他系统的监听。
...
> **注意**：源码中**没有** `UnRegisterAll<T>()` 或 `UnRegisterAll()` 方法。清除事件请用 `RemoveEventListener`（单个）、`RemoveAllListeners`（按事件 ID 清空）、`GameEventMgr.Clear()`（局部批量）或 `GameEvent.Shutdown()`（全局，仅退出时）。
...
| `GameEvent.HasEventListener` | 0~6 个泛型参数 + Delegate | 0~5 个泛型参数 + Delegate |
| `GameEvent.RemoveAllListeners` | 无泛型（仅事件 ID） | 无泛型（仅事件 ID） |
...
GameEvent.RemoveAllListeners(eventType);            // 按事件 ID 清空全部监听
