...
> **当前 Fork 排障入口**：先确认程序集包名、`RuntimePackages` 构建管线以及 `AOT/HotDll/PDB` 子目录。Archive 管线检查 `RawFileObject` 和归档解密器，非 Archive 管线再检查 `TextAsset`。不要只按旧 `AssemblyTextAssetPath` 排查。参见 [当前 Fork 定制功能](../../项目概述/当前Fork定制功能.md)。
