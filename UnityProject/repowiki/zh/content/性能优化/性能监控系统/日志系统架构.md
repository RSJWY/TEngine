...
## Fork 扩展：TouchSocket 桥接与早期日志缓冲

> 本节为 fork 定制改动，与上游 TEngine 不同。通用框架原理见上文，fork 改动细节以 [Books/Fork/logging.md](../../../../../Books/Fork/logging.md) 为准。

### TouchSocket 日志桥接与落盘

`UnityLoggerBridge`（`Assets/TEngine/Runtime/Core/Log/UnityLoggerBridge.cs`）在 `[RuntimeInitializeOnLoadMethod(BeforeSplashScreen)]` 自动初始化，订阅 `Application.logMessageReceivedThreaded`、`TaskScheduler.UnobservedTaskException`、`UniTaskScheduler.UnobservedTaskException`，经 TouchSocket `FileLogger` 落盘到 `persistentDataPath/Logs/yyyy-MM-dd/`，单文件 1 MB 滚动，保留最近 3 天。`TouchSocketContainerUnityDebugLogger` 将 TouchSocket 日志按级别映射到 `Debug.Log` / `LogWarning` / `LogError`，统一带 `[TouchSocket]` 前缀。

### 早期日志缓冲 EarlyLogBuffer

`UnityLoggerBridge` 在 `BeforeSplashScreen` 才就绪，而 Obfuz 静态密钥初始化等早期流程在 `AfterAssembliesLoaded` 阶段（更早）已经执行。这一阶段的 `Log.Info` / `Log.Fatal` 虽然会进 Unity Console 和 Player.log，但不会被 TouchSocket 文件日志捕获。

Fork 新增 `EarlyLogBuffer`（`Assets/TEngine/Runtime/Core/Log/EarlyLogBuffer.cs`）和 `Log.EarlyInfo` / `Log.EarlyWarning` / `Log.EarlyError` 三个专用 API：

- 调用时先 `EarlyLogBuffer.Enqueue` 留副本，再走对应级别的常规路径输出到 Console。
- `UnityLoggerBridge.Init()` 末尾 `Flush(s_FileLogger)`：把缓冲补写到文件日志（带 `[Early]` 前缀），然后关闭缓冲区。
- `Flush` 后 `s_Closed = true`，后续 `Enqueue` 直接丢弃（no-op），避免重复落盘——Init 之后调 `Log.EarlyInfo` 等价于 `Log.Info`。
- `UnityLoggerBridge.Shutdown()` 调 `Reset()` 清空缓冲并重新开放，兼容 Domain Reload。

| 调用时机 | Console | Player.log | TouchSocket 文件 | Debugger 窗口 |
| --- | --- | --- | --- | --- |
| `UnityLoggerBridge.Init` **之前** | ✅ | ✅ | ✅（Flush 补写） | ❌ |
| `UnityLoggerBridge.Init` **之后** | ✅ | ✅ | ✅（正常落盘） | ✅ |

> 仅建议启动早期、低频使用；正常运行期用常规 `Log.Info` / `Warning` / `Error` / `Fatal`。详见 [Books/Fork/logging.md](../../../../../Books/Fork/logging.md)。