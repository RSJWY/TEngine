...
> **当前 Fork 基线**：项目默认不使用 Luban。部署配置、工具配置和小型业务配置使用 `GameModule.Config`（`RuntimeConfigModule`）加载 `Assets/StreamingAssets/Configs/` 下的 TOML/JSON。默认清单为 `config_manifest.toml`（强制 TOML，不再兼容 JSON 清单）；消费方优先使用 `TryGet`/`TryGetText` 并提供默认值。本文后续 Luban 内容仅作为可选配置表方案参考，不代表当前项目默认架构。总览见 [当前 Fork 定制功能](../项目概述/当前Fork定制功能.md)，详细变更见 [runtime-config.md](../../../../../Books/Fork/runtime-config.md)。
