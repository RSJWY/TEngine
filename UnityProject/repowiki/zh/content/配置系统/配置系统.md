...
> **当前 Fork 基线**：项目默认不使用 Luban。部署配置、工具配置和小型业务配置使用 `GameModule.Config`（`RuntimeConfigModule`）加载 `Assets/StreamingAssets/Configs/` 下的 TOML/JSON，读取走覆盖链（`persistentDataPath/Configs` 覆盖 `StreamingAssets/Configs`，可用于打包后现场覆盖）。默认清单为 `config_manifest.toml`（强制 TOML，不再兼容 JSON 清单）；消费方优先使用 `TryGet`/`TryGetText` 并提供默认值。本文后续 Luban 内容仅作为可选配置表方案参考，不代表当前项目默认架构。总览见 [当前 Fork 定制功能](../项目概述/当前Fork定制功能.md)，详细变更见 [runtime-config.md](../../../../../Books/Fork/runtime-config.md)。
...
## 结论
当前 fork 的轻量配置入口是 `RuntimeConfigModule`；Luban 仅在项目明确需要强类型配置表和转表流程时作为可选方案使用。两者面向不同规模的数据，不应为普通部署配置或 UI 功能主动引入 Luban。
