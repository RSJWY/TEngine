...
> **当前 Fork 基线**：统一入口为 `Build/打包工具窗口`，运行时包与构建工具共同读写 `UpdateSetting.RuntimePackages`。默认流程为“编译热更 DLL -> 构建资源包 -> 发布整理 -> 最小包处理 -> 构建 Player -> Inno Setup”；Windows/Linux 及资源发布产物位于 `Releases/`，Android/iOS/MacOS/WebGL Player 仍位于 `Output/Player/`。AssetBundle 或 Player 构建失败必须阻断安装包阶段。总览见 [当前 Fork 定制功能](../项目概述/当前Fork定制功能.md)，详细变更见 [resource-build.md](../../../../../Books/Fork/resource-build.md)。
