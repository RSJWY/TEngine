...
> **当前 Fork 基线**：热更程序集位于独立程序集包，实际包名通过 `UpdateSetting.GetAssemblyPackageName()` 获取。程序集包默认使用 `ArchiveFileBuildPipeline`；归档管线返回 `RawFileObject`，非归档管线才继续使用 `TextAsset.bytes`。目录由 `UpdateSetting` 配置为 `AOT/`、`HotDll/`、`PDB/`，不要再依赖固定 `AssemblyTextAssetPath`。总览见 [当前 Fork 定制功能](../项目概述/当前Fork定制功能.md)，详细变更见 [hot-update.md](../../../../../Books/Fork/hot-update.md)。
