...
> **当前 Fork 基线**：热更 DLL、PDB 和 AOT 元数据由独立程序集包管理，目录通过 `UpdateSetting` 配置。Archive 管线读取 `RawFileObject`，其他管线兼容 `TextAsset`；固定 `AssemblyTextAssetPath` 仅属于旧版描述。参见 [当前 Fork 定制功能](../项目概述/当前Fork定制功能.md)。
