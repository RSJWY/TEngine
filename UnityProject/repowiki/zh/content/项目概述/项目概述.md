...
> **当前 Fork 提示**：本仓库已升级到 YooAsset 3.x，并新增运行时配置、场景、窗口、GameObjectPool、动画、DataBinding、存档和构建发布能力。阅读通用架构前建议先看 [当前 Fork 定制功能](当前Fork定制功能.md)。
...
Mode --> |编辑器模拟| Sim["EditorSimulateModeOptions"]
Mode --> |单机| Offline["OfflinePlayModeOptions"]
Mode --> |主机| Host["HostPlayModeOptions"]
Mode --> |WebGL| Web["WebPlayModeOptions"]
Sim --> Init["初始化并设置默认包"]
Offline --> Init
Host --> Init
Web --> Init
