...
| 构建工具 | 使用 `Build/打包工具窗口`，运行时包配置与构建配置共用 `UpdateSetting.RuntimePackages`；外置命令行/GUI 构建走 `UnityProject/BuildCLI/`（Python，复用同一套 `ReleaseTools` 链路，见 [Fork构建与调试工具](../编辑器工具/Fork构建与调试工具.md)） |
