...
Assets/GameScripts/HotFix/GameLogic/GameModule.cs  新增 GameModule.Screen 访问器（业务显式调用时创建模块）
```
...
### 2. 标准 `GetModule<IScreenModule>()` 注册，业务手动激活

模块在 AOT 层，`Type.GetType("TEngine.ScreenModule, TEngine.Runtime")` 可正常解析，无需 `RegisterModule` 预创建实例。  
首次访问 `GameModule.Screen` 时会按 TEngine 模块系统创建实例并调用 `OnInit`，但 `OnInit` 保持空实现，不读取配置、不应用布局。窗口布局只在业务显式调用 `GameModule.Screen.ApplyAll()` / `ApplyScreen()` / `SetTopmost()` 时工作。
...
业务确认需要窗口布局
  └─ GameModule.Screen.ApplyAll()
       └─ ModuleSystem.GetModule<IScreenModule>()（首次访问时创建模块，OnInit 为空）
            └─ ApplyAllAsync()
...
- `Assets/GameScripts/HotFix/GameLogic/GameModule.cs`（访问入口）
