...
namespace GameLogic
{
    ...
    /// </summary>
    /// <remarks>
    /// 维护存档实例缓存、特性反射缓存、key冲突校验表。
    /// </remarks>
    public class ClientSaveDataMgr : Singleton<ClientSaveDataMgr>, IUpdate
    {
        ...
        private ClientSaveDataAttribute GetSaveDataAttribute<T>() where T : BaseClientSaveData, new()
        {
            ...
            if (!m_cacheAttributeDict.TryGetValue(type, out var attr))
            {
                ...
                if (attr == null)
                {
                    throw new InvalidOperationException($"未标记 SaveData: {type.Name}");
                }
        ...
        private string GetStorageKey(Type type, ClientSaveDataAttribute attr)
        {
            ...
            {
                throw new InvalidOperationException($"ClientSaveData key冲突: key={storageKey}, type={type.Name}, cacheType={cacheType.Name}");
            }
