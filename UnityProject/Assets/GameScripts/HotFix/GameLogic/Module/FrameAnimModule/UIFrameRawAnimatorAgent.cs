...
{
    /// <summary>
    /// UI帧动画播放代理（RawImage 版）。绑定 UGUI <see cref="RawImage"/> 显示序列帧。
    /// <para>与 <see cref="UIFrameAnimatorAgent"/>（Image 版）逻辑同构，
    /// 区别仅在显示侧：<c>rawImage.texture = sprite.texture</c>。</para>
    /// <para>注意：Sprite 经图集打包后多张共享同一 Texture2D，用 RawImage 会把整张大图当 texture 显示，
    /// 故本代理适用于「每帧独立 PNG、不走图集打包」的场景。</para>
    /// </summary>
    public sealed class UIFrameRawAnimatorAgent : MemoryObject
    {
        #region 字段
        ...
        /// <summary>
        /// 是否已经绑定显示用的 RawImage 组件。
        /// </summary>
        private bool _isBindDisplayRawImage;
        ...
        /// <summary>
        /// 是否有效（未销毁、已初始化、已绑定RawImage）
        /// </summary>
        public bool IsValid => !_isDestroy && _isInit && _rawImage != null;
        ...
        /// 创建帧动画代理实例
        /// </summary>
        public static UIFrameRawAnimatorAgent Create()
        {
            return MemoryPool.Alloc<UIFrameRawAnimatorAgent>();
        }
        ...
        public void BindDisplayRender(RawImage rawImage)
        {
            if (_isBindDisplayRawImage)
            {
                return;
            }
            _isBindDisplayRawImage = true;
            _rawImage = rawImage;
            SetFirstFrame();
        }
        ...
        private void SetFirstFrame()
        {
            ...
            if (_isSetFirstFrame || _rawImage == null)
            {
                return;
            }
        ...
        /// <summary>
        /// 设置当前帧图片到 RawImage 组件（取 Sprite 的底层 Texture2D）。
        /// </summary>
        /// <param name="sprite">待显示的帧图片。</param>
        private void SetSprite(Sprite sprite)
        {
            if (_isDestroy || !_isInit || _rawImage == null || sprite == null)
            {
                return;
            }
        ...
        private void SetImageSize(bool revert = false)
        {
            ...
            _rawImage.transform.localScale = revert ? Vector3.one : _uiModelScale;
        }
