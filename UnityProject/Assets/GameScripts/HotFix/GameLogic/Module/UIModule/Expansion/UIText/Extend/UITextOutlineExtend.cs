    ...
    public static class OutlineMaterialCache
    {
        ...
        public static Material GetOrCreateMaterial(string shaderName, Texture fontTexture)
        {
            ...
            if (s_baseMaterial == null)
            {
                try
                {
                    s_baseMaterial = GameModule.Resource.LoadAsset<Material>(UITextMaterialAddress);
                }
                catch (System.Exception e)
                {
                    Log.Error($"[OutlineMaterialCache] Failed to load material from YooAsset (address: {UITextMaterialAddress}): {e.Message}");
                    return null;
                }
            }
            ...
            if (s_baseMaterial == null)
            {
                Log.Error($"[OutlineMaterialCache] Material not found from YooAsset address: {UITextMaterialAddress}");
                return null;
            }
        ...
        private void UpdateOutLineMaterial()
        {
            ...
            if (!Application.isPlaying)
            {
                if (m_text.material == m_text.defaultMaterial)
                {
                    ...
                    if (material == null)
                    {
                        Log.Error("Text Out Line Material Not Find Please Check Material Path!");
                    }
                    else
                    {
