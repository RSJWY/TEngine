...
{
    [Serializable]
    public class UIRawImageMaskExtend
    {
        ...
        {
	        m_rawImage = rawImage;
	        m_rectTransform = m_rawImage.rectTransform;
        ...
        public void Update()
        {
	        if (m_rawImage == null || m_rectTransform == null) return;
        ...
        public void OnPopulateMesh(VertexHelper vh)
        {
	        ...
	        if (m_rawImage != null && m_rectTransform != null)
	        {
		        this.m_ringWidth = (float)Mathf.Clamp(m_ringWidth, 0, m_rectTransform.rect.width / 2f);
	        }
	        ...
	        //RawImage 无 Sprite 概念，UV 直接由 uvRect 换算
	        Rect uvRect = m_rawImage.uvRect;
	        Vector4 uv = m_rawImage.texture != null
		        ? new Vector4(uvRect.x, uvRect.y, uvRect.x + uvRect.width, uvRect.y + uvRect.height)
		        : Vector4.zero;
	        ...
	        if (m_fill) //圆形
	        {
	            ...
	            uiVertex.color = m_rawImage.color;
